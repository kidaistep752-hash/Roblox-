...
_G.ShotButton.TextStrokeTransparency = 0.5
...
MM2Title.TextStrokeTransparency = 0.5
...
function _G.createTab(name)
    ...
    TabBtn.TextStrokeTransparency = 0.5
    TabBtn.ZIndex = 12
...
function _G.createFeatureWithLeftSlider(parent, configKey, speedKey, text, min, max)
    ...
    Label.Size = UDim2.new(1, -70, 1, 0); Label.Position = UDim2.new(0, 35, 0, 0); Label.BackgroundTransparency = 1; Label.Text = text; Label.TextColor3 = Color3.fromRGB(200, 200, 200); Label.TextXAlignment = Enum.TextXAlignment.Left; Label.Font = Enum.Font.SourceSans; Label.TextSize = 12; Label.TextStrokeTransparency = 0.5; Label.ZIndex = 15

    local GearBtn = Instance.new("TextButton", MainLine)
    GearBtn.Size = UDim2.new(0, 25, 0, 25); GearBtn.Position = UDim2.new(1, -30, 0, 2); GearBtn.BackgroundTransparency = 1; GearBtn.Text = "⚙️"; GearBtn.TextSize = 12; GearBtn.ZIndex = 15
    ...
    ValueLabel.Size = UDim2.new(0, 30, 0, 20); ValueLabel.Position = UDim2.new(1, -35, 0, 5); ValueLabel.BackgroundTransparency = 1; ValueLabel.Text = tostring(_G.Config[speedKey]); ValueLabel.TextColor3 = Color3.fromRGB(255, 255, 255); ValueLabel.Font = Enum.Font.SourceSansBold; ValueLabel.TextSize = 11; ValueLabel.TextStrokeTransparency = 0.5; ValueLabel.ZIndex = 15
...
mm2SubTabBtn.Text = "Open MM2 Menu"
mm2SubTabBtn.TextColor3 = Color3.fromRGB(186, 85, 211)
mm2SubTabBtn.Font = Enum.Font.SourceSans
mm2SubTabBtn.TextSize = 12
mm2SubTabBtn.TextStrokeTransparency = 0.5
...
_G.TpGunButton.TextStrokeTransparency = 0.5
...
local rolesCheat = _G.createCheckbox(_G.MM2Content, "MM2Roles", "Roles Chams (Red/Blue/White)")
local gunEspCheat = _G.createCheckbox(_G.MM2Content, "MM2Gun", "Gun Chams + 3D Text")
local shotCheat = _G.createCheckbox(_G.MM2Content, "MM2Shot", "Auto Shot Murder")
local tpGunToggle = _G.createCheckbox(_G.MM2Content, "Target", "TP to Gun Button")
local flingCheat = _G.createCheckbox(_G.pOther, "OnePunchFling", "OnePunchFling FLING")
...
-- ====================================================================
--  PART 5.3: МОЩНЫЙ FLING (ТОЛЬКО ВРАГА ОТТАЛКИВАЕМ)
-- ====================================================================
RunService.Heartbeat:Connect(function()
    ...
    if targetRoot then
        ...
        -- ТОЛЬКО ВРАГА ОТТАЛКИВАЕМ, НЕ СЕБЯ!
        targetRoot.AssemblyLinearVelocity = direction * 500 + Vector3.new(0, 300, 0)
        
        task.wait(0.02)
        targetRoot.AssemblyLinearVelocity = direction * 600 + Vector3.new(0, 350, 0)
...
-- ====================================================================
--  PART 5.4: НОВЫЙ АВТО-ВЫСТРЕЛ (ПОЛНОСТЬЮ ПЕРЕДЕЛАН)
-- ====================================================================
_G.ShotButton.MouseButton1Click:Connect(function()
    if not _G.Config.MM2Shot or not LocalPlayer.Character then
        print("Error: MM2Shot disabled or no character")
        return 
    end
    ...
    if not myRoot or not hum then
        print("Error: No root or humanoid")
        return 
    end
    ...
    -- Экипируем пистолет если в рюкзаке
    ...
    if not murderer then
        print("Error: No murderer found")
        return 
    end
    
    print("Found murderer, aiming and shooting...")
    
    -- Поворачиваемся к убийце
    local targetPos = murderer.Position
    myRoot.CFrame = CFrame.new(myRoot.Position, targetPos)
    
    task.wait(0.08)
    
    -- СТРЕЛЯЕМ разными способами
    local fired = false
    
    -- Способ 1: Gun.Fire RemoteEvent
    if gun:FindFirstChild("Fire") and gun.Fire:IsA("RemoteEvent") then
        pcall(function()
            gun.Fire:FireServer(targetPos)
            fired = true
            print("Fired method 1: Fire RemoteEvent")
        end)
    end
    
    -- Способ 2: Gun.Shoot RemoteEvent
    if not fired and gun:FindFirstChild("Shoot") and gun.Shoot:IsA("RemoteEvent") then
        pcall(function()
            gun.Shoot:FireServer(targetPos)
            fired = true
            print("Fired method 2: Shoot RemoteEvent")
        end)
    end
    
    -- Способ 3: Gun:Activate()
    if not fired then
        pcall(function()
            gun:Activate()
            fired = true
            print("Fired method 3: Activate")
        end)
    end
...
-- ====================================================================
--  PART 5.6: ЗАКРЫТИЕ MM2 МЕНЮ С КРЕСТИКОМ
-- ====================================================================
...
MM2CloseBtn.TextStrokeTransparency = 0.5
...
MM2CloseBtn.MouseEnter:Connect(function()
    MM2CloseBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 100)
end)

MM2CloseBtn.MouseLeave:Connect(function()
    MM2CloseBtn.BackgroundColor3 = Color3.fromRGB(255, 50, 50)
end)
...
-- ====================================================================
--  PART 5.7: SYSTEM BUTTONS (С ПРИВАТНЫМ СЕРВЕРОМ)
-- ====================================================================
...
RejoinBtn.Text = "New Private Server"
RejoinBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
RejoinBtn.Font = Enum.Font.SourceSans
RejoinBtn.TextSize = 12
RejoinBtn.TextStrokeTransparency = 0.5
...
RejoinBtn.MouseButton1Click:Connect(function()
    -- Создаём новый приватный сервер
    pcall(function()
        TeleportService:ReserveServer(game.PlaceId)
    end)
    
    -- Если не сработало, просто перезагружаемся
    task.wait(0.5)
...
print("✅ SimpleNeonSense Script Loaded!")
